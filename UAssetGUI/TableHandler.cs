            ...
            for (int i = 0; i < asset.Exports.Count; i++)
            {
                ...
                switch (baseUs)
                {
                    ...
                    case NormalExport us:
                    {
                        ...
                        if (us is SoundCueExport uscue) {
                            //uscue.PopulateSoundCueInfo();
                            var parentNode2 = new PointingTreeNode("SoundCue Data", uscue.GetCueGraph());
                            categoryNode.Nodes.Add(parentNode2);
                        }
                            ...
                            if (us is StructExport structUs)
                        {
                            ...
                            {
                                var bytecodeNode = new PointingTreeNode("ScriptBytecode (" + structUs.ScriptBytecode.Length + " instructions)", structUs.ScriptBytecode, PointingTreeNodeType.Bytecode);
                                parentNode2.Nodes.Add(bytecodeNode);
                            }
                         }
        ...
        private void InterpretThing(PropertyData me, PointingTreeNode ourNode)
        {
            ...
            switch (me.PropertyType.Value.Value)
            {
                ...
                case "FieldPathProperty":
                    var fp = (FieldPathPropertyData)me;
                    ourNode.Nodes.Add(new PointingTreeNode(fp.Name.Value.Value + " (" + fp.Value.Length + ")", fp.Value));
                    //var fpathNode = new PointingTreeNode(fp.Name.Value.Value + " (" + fp.Value.Length + ")", fp.Value);
                    //ourNode.Nodes.Add(fpathNode);
                    break;
                case "Box":
                    ...
                    else
                    {
                        switch (thisPD.PropertyType.Value.Value)
                        {
                            ...
                            case "MulticastDelegateProperty":
                                ...
                                break;
                            case "FieldPathProperty":
                                var fpdata = (FieldPathPropertyData)thisPD;
                                row.Cells[4].Value = fpdata.ResolvedOwner.Index;
                                break;
                            case "LinearColor":
            ...
            switch (mode)
            {
                ...
                case TableHandlerMode.NameMap:
                    ...
                    for (int num = 0; num < headerIndexList.Count; num++)
                    {
                        ...
                        dataGridView1.Rows[num].HeaderCell.Value = num.ToString("X");//Convert.ToString(num);
                    }
                    ...
                    if (listView1.SelectedNode is PointingTreeNode pointerNode)
                    {
                        ...
                        switch (pointerNode.Pointer)
                        {
                            ...
                            case FMulticastDelegate[] usRealMDArr:
                                ...
                                standardRendering = false;
                                break;
                            case FName[] usNamesArr:
                                AddColumns(new string[] { "Name", "" });
                                for (int i = 0; i < usNamesArr.Length; i++) {
                                    dataGridView1.Rows.Add(usNamesArr[i].ToName());
                                }
                                standardRendering = false;
                                break;
                            ...
                            case string[] graph:
                                ...
                                dataGridView1.Visible = false;
                                jsonView.Text = "";
                                jsonView.Text = String.Join(Environment.NewLine, graph);
                                jsonView.Visible = true;
                                currentlyFocusedControl.Focus();
                                origForm.ForceResize();
                                standardRendering = false;
