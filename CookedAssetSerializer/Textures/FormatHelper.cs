...
namespace Textures {
    public class FormatHelper {
        ...
        public static bool HasFormatTableKey(EPixelFormat format) {
            return format_table.ContainsKey(GetBaseFormat(format));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int GetBlockWidth(EPixelFormat format) {
            if (!HasFormatTableKey(format)) return 8;
            return format_table[GetBaseFormat(format)].BlockWidth;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int GetBlockHeight(EPixelFormat format) {
            if (!HasFormatTableKey(format)) return 8;
            return format_table[GetBaseFormat(format)].BlockHeight;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int GetBytesPerPixel(EPixelFormat format) {
            if (!HasFormatTableKey(format)) return 8;
            return format_table[GetBaseFormat(format)].BytesPerPixel;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int GetBlockDepth(EPixelFormat format) {
            if (!HasFormatTableKey(format)) return 8;
            return format_table[GetBaseFormat(format)].BlockDepth;
        }
        ...
        private static string GetBaseFormat(EPixelFormat format) {
            string[] items = {
                "_UNORM", "_SRGB", "_SINT", "_SNORM", "_UINT", "_UFLOAT", "_SFLOAT", "_FLOAT", "H_SF16", "H_UF16", "PF_"
            };
            var output = format.ToString();
            return items.Aggregate(output, (current, t) => current.Replace(t, string.Empty));
        }

        private static readonly Dictionary<string, FormatInfo> format_table = new() {
        ...
        private class FormatInfo {
            ...
            public FormatInfo(int bytesPerPixel, int blockWidth, int blockHeight, int blockDepth,
